 ...
 * Description: functions for CLATCH descriptors
...
const int FCLATCH::L=64;

void FCLATCH::meanValue(const std::vector<FCLATCH::pDescriptor> &descriptors, 
  FCLATCH::TDescriptor &mean)
{
  ...
  else
  {
    ...
    mean = cv::Mat::zeros(1, FCLATCH::L, CV_8U);
...
int FCLATCH::distance(const FCLATCH::TDescriptor &a,
  const FCLATCH::TDescriptor &b)
{
...
std::string FCLATCH::toString(const FCLATCH::TDescriptor &a)
{
  stringstream ss;
...
void FCLATCH::fromString(FCLATCH::TDescriptor &a, const std::string &s)
{
  a.create(1, FCLATCH::L, CV_8U);
  unsigned char *p = a.ptr<unsigned char>();
  
  stringstream ss(s);
  for(int i = 0; i < FCLATCH::L; ++i, ++p)
  {
    int n;
    ss >> n;
...
void FCLATCH::toMat32F(const std::vector<TDescriptor> &descriptors, 
  cv::Mat &mat)
{
  if(descriptors.empty())
  {
  ...
  mat.create(N, FCLATCH::L*8, CV_32F);
