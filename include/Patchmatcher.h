...
public:
    ...
    Patchmatcher(float nnratio=0.6, bool checkOri=true);
    ...
    // Search matches between Frame keypoints and last frame. Returns number of matches
    // Used to track the local map (Tracking)
    int SearchByHomography(Frame &CurrentFrame, const Frame &LastFrame,
            const cv::Mat& Homography, const float th, const bool bMono);

    // Search matches between Frame keypoints and last frame. Returns number of matches
    // Used to track the local map (Tracking)
    int SearchByHomography(Frame &CurrentFrame, KeyFrame &LastFrame,
            const cv::Mat& Homography, const float th, const bool bMono);
...
protected:
    ...
    void  VFCFilter(const vector<cv::KeyPoint> &keypointsA,const  vector<cv::KeyPoint> &keypointsB, vector<DMatch> &matches, vector<DMatch> &correctMatches);
